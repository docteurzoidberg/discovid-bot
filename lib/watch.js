...
module.exports = (id, movie) => {
    ...
    Watcher.start = () => {
        ...
        Watcher.emit('started', Watcher.id, Watcher.movie);
    };
    ...
    Watcher.stop = () => {
        ...
        Watcher.emit('stopped', Watcher.id, Watcher.movie);
    };
    ...
    Watcher.watch = async () => {
        ...
        if(!moviefiledata || moviefiledata.length === 0) {
            ...
            Watcher.movie.movieFile = moviefiledata[0];
            Watcher.emit('downloaded', Watcher.id, Watcher.movie);
            Watcher.stop();
        }
